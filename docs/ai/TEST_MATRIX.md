...
| Authenticated HTTPS route distribution    | ✅   | ✅          | —   | Strict bounded codec/protocol tests cover canonical full catalogs; real mTLS verifies bootstrap, complete-catalog replacement, stale/expired fail-closed state, and same-address recovery. Edge tests verify atomic dynamic resolution and CLI source exclusivity. |
