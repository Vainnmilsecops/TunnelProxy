...
| ----------------------------------------- | ---- | ----------- | --- | --------------------------------------------------------------------------------------------------------------------------- |
| Expiring signed access URLs               | ✅   | ✅          | ✅  | Strict codec/key-ring tests cover tampering, host/key binding, lifetime/skew and bounds; offline CLI generates, merges, publishes, and signs; real TLS proves fail-closed admission, stripping, overlap rotation, key retirement, and listener continuity. |
| Signed-access key-ring live reload        | ✅   | ✅          | ✅  | Digest/generation tests cover overlap, invalid-candidate last-known-good retention, and removal; Control Plane publishing is keyring-before-manifest; real TLS rotates key 1 to key 1+2 to key 2 without restarting Edge. |
...
| Edge: TCP relay activity idle timeout     | ✅   | ✅          | —   | Echo and relay integration tests prove activity resets the shared deadline beyond one idle period, silence returns a typed timeout, and fixed-buffer half-close behavior remains covered. |
| Bidirectional streaming                   | ✅   | ✅          | —   | Session 03 deliverables.                                                                                                    |
| Forwarder: explicit forwarding config     | ✅   | ✅          | —   | `ForwardConfig::validate` rejects zero capacity/connect timeout and relay idle timeouts outside 1 ms..=1 hour; explicit per-IP construction rejects zero or values above global capacity with typed `ForwardConfigError`. |
...
| Forwarder: per-source-IP admission        | ✅   | ✅          | —   | Unit coverage proves independent bounded peer buckets and RAII reclamation; real TCP proves same-IP rejection occurs before upstream dial, preserves the admitted relay, and releases after idle timeout. |
...
| Forwarder: structured error categories    | ✅   | ✅          | —   | `ForwardError::category()` includes fixed capacity, connect, relay-I/O, and `relay_idle_timeout` outcomes; integration coverage proves timeout releases its RAII permit. |
| Edge: TCP forwarder CLI                   | ✅   | —           | —   | Example-target tests cover relay-idle and per-IP defaults, parsing/help, and strict timeout/global admission bounds. |
| HTTPS/HTTP/1.1 reverse proxy              | ✅   | ✅          | —   | Real TLS crosses exact Host/SNI routing through Edge → Agent → local HTTP; two requests reuse one TLS connection and the hard request cap closes it. |
| Bounded HTTP/2 public ingress             | ✅   | ✅          | —   | Unit/parser tests cover opt-in ALPN and strict stream/keepalive policy; real TLS multiplexes two streams, isolates body-limit/timeout failure, rejects authority fronting, translates to local HTTP/1.1, falls back to HTTP/1.1, reports bounded telemetry, and drains an idle h2 connection. |
| Bounded HTTP/1.1 WebSocket upgrade        | ✅   | ✅          | —   | Config/relay tests cover strict limits, bidirectional bytes, and idle timeout; real TLS crosses Edge → Agent → local service, validates both handshakes/subprotocol, relays text/binary/ping bytes, rejects malformed/fronted/capacity/forged-local attempts, releases capacity, reports fixed telemetry, and force-drains a stalled upgrade. |
| Bounded route-bound HTTP/1.1 CONNECT      | ✅   | ✅          | —   | Config/parser/relay/metrics tests cover opt-in limits and fixed telemetry; two real TLS Edge → Agent → local E2Es prove exact authority-port/Host/SNI routing, byte-exact opaque relay, body/fronting/capacity rejection and release, idle timeout, and forced drain without arbitrary destination dialing. |
| Bounded route-bound classic HTTP/2 CONNECT | ✅  | ✅          | —   | Config/parser/metrics tests cover independent opt-in, HTTP/2 dependency, shared admission, and fixed telemetry; two real TLS h2 E2Es prove concurrent upgraded streams, byte-exact relay, request half-close, wrong port/fronting/extended-CONNECT rejection, capacity release, idle timeout, stream isolation, GOAWAY ownership, and forced drain. |
| Bounded route-bound RFC 8441 WebSocket | ✅  | ✅          | —   | Config/parser/metrics tests cover independent opt-in, conditional extended-CONNECT advertisement, shared WebSocket admission, and fixed telemetry; two real TLS h2 E2Es prove concurrent streams, generated-key local HTTP/1.1 handshake translation, subprotocol/opaque frame relay, extension/fronting/cross-protocol capacity rejection, idle release, GOAWAY ownership, and forced drain. |
...
| HTTP ingress resource bounds             | ✅   | ✅          | —   | Config/CLI validate global/per-IP connections and request rates, bounded rate-peer state, header bytes/count, body, TLS/header/per-request deadlines, keep-alive request cap, duplex capacity, and drain. |
...
| HTTP rate-limit response and isolation   | ✅   | ✅          | —   | `429` includes integer `Retry-After`; one reused TLS connection proves per-request admission, rejection-close, and that rejected work never reaches the local service. |
| HTTP keep-alive deadline and shutdown    | ✅   | ✅          | —   | Unit coverage enforces the 1..=1024 cap and response-body deadline; real TLS proves request timeout to `504`, cap-close, metrics, graceful idle drain, and forced active-request shutdown at the deadline. |
...
| Agent operations and connection metrics  | ✅   | ✅          | —   | Config/render tests enforce loopback and fixed labels; subprocess/real TCP covers pre-dial bind rollback, offline→connected→reconnect readiness, counters, capacity RAII, ordered drain, and port release. |
| Multiplexed fairness/capacity telemetry   | ✅   | ✅          | —   | Atomic/RAII tests cover admission waits, pipeline depth/capacity, multi-session aggregation, active streams, byte/frame counters, resets, and burst yields; real TCP verifies live capacity, disconnect/reconnect release, exact Agent/Edge directional bytes, and no identity or payload labels. |
| Control Plane operations and service metrics | ✅ | ✅       | —   | Config/render tests enforce loopback and fixed labels; real TCP plus an mTLS snapshot client prove readiness, service metrics, bind rollback, ordered shutdown, and listener release without scrape-time SQLite access. |
...
| Bounded redacted HTTPS request history   | ✅   | ✅          | —   | Registry/parser/operations tests cover capacity, eviction, UTF-8-safe path truncation, sequence exhaustion, 64 KiB JSON output, fixed outcome metrics, and disabled compatibility; real TLS HTTP/1.1/HTTP/2 covers redaction, exclusion, timeout/rejection classification, protocol attribution, no-store GET/HEAD, method rejection, and listener drain. |
| Request-history cursor pagination        | ✅   | ✅          | —   | Strict parser tests cover defaults, 1..128 limits, exclusive non-zero cursors, and malformed/duplicate/unknown rejection; unit and real HTTP/1.1/HTTP/2 tests cover newest-first continuation without duplicates, eviction gaps, additive metadata, disabled 404 compatibility, and the unchanged 64 KiB ceiling. |
...
| Process logging contract                  | ✅   | ✅          | —   | Pure tests cover format/filter/buffer bounds, FIFO, drop-newest pressure, whole-event size rejection, sink failure, healthy drain, and blocked-writer deadline; all three binaries cover buffered text/JSON drain, schema, filtering, stdout/stderr separation, pre-mutation failure, and secrecy. |
...
| Request inspection                        | ✅   | ✅          | —   | Edge inspects only routing/security metadata for HTTP/1.1 and HTTP/2; application payload inspection, access rules, and replay remain deferred. |
| Shared public hostname identity           | ✅   | —           | —   | Canonical case/trailing-dot handling plus IP, port, wildcard, ASCII-label, label-length, and total-length rejection are unit tested; Edge retains its compatible hostname API. |
| Durable HTTPS route catalog               | ✅   | ✅          | —   | SQLite tests cover transactional versioning, idempotency, deterministic order, capacity/version exhaustion rollback, corruption, reopen, and coexistence with snapshot state; subprocess CLI tests cover validation before mutation and upsert/list/remove output. |
| Managed hostname lifecycle                | ✅   | ✅          | —   | Repository tests cover canonical base-domain bounds, 128-bit label shape, idempotent restart, ownership conflicts, collision retry/exhaustion, entropy/version rollback, release, and legacy migration; CLI tests cover pre-mutation validation and stable allocate/release output, while the real route stream observes live allocation and removal. |
| Authenticated Agent hostname lifecycle    | ✅   | ✅          | —   | Dedicated protocol tests reject malformed/noncanonical messages; parser tests enforce complete service/client configuration; a real mTLS test covers exact snapshot authorization, unknown identities, wrong bindings, idempotent allocation, durable-before-live route publication, and release. |
| Agent hostname TLS generation reload      | ✅   | ✅          | —   | Parser tests enforce listener/manifest and paired identity-path configuration; real mTLS rotates server identity and Agent CA, rejects the old client, retains generation 2 after an invalid generation 3, and proves last-known-good expiry is terminal. |
| Managed HTTP Agent orchestration          | ✅   | ✅          | —   | Parser/subprocess tests cover port, conflicting local target, complete dual-service mTLS, help, and exit contracts; real mTLS/HTTPS allocates a dynamic wildcard route, registers the exact Agent/Tunnel, forwards HTTP to loopback, reuses the hostname/version, and retains ownership after Agent shutdown. |
| Authenticated HTTPS route distribution    | ✅   | ✅          | —   | Strict bounded codec/protocol tests cover canonical full catalogs; real mTLS verifies bootstrap, complete-catalog replacement, stale/expired fail-closed state, same-address recovery, TLS rotation, and last-known-good rollback. Edge tests verify atomic dynamic resolution and CLI source exclusivity. |
| Managed HTTP public reachability          | ✅   | ✅          | —   | Strict challenge codec and CLI/config tests cover canonical bounds, opt-in, CA validation, timeout, and cancellation; real TLS covers successful exact-route proof, wrong CA, signed-access coexistence, rate admission, and no local forwarding. |
